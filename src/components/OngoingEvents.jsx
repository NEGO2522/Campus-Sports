                  ...
                  >
                    <div className="p-3 sm:p-4">
                      <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-1.5 xs:gap-2 mb-2">
                        ...
                        <div className="text-[10px] xs:text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                          {event.sport}
                        </div>
                      </div>
                        ...
                        {matchesByEvent[event.id] && matchesByEvent[event.id].length > 0 ? (
                          matchesByEvent[event.id].map((match) => (
                            <div key={match.id} className="mb-2 last:mb-0">
                              {match.matchName && (
                                <div className="font-bold text-orange-600 text-xs xs:text-sm mb-1 truncate">
                                  {match.matchName}
                                </div>
                              )}
                                ...
                                <div className="text-sm xs:text-base text-black font-medium space-y-1.5">
                                  <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-1">
                                    ...
                                    {match.matchStarted === true ? (
                                      <div className="flex items-center text-red-600 font-bold text-[10px] xs:text-xs whitespace-nowrap">
                                        <span className="w-1.5 h-1.5 xs:w-2 xs:h-2 bg-red-500 rounded-full mr-1.5 animate-pulse"></span>
                                        LIVE
                                      </div>
                                    ) : (
                                      match.dateTime && (
                                        <div className="text-[10px] xs:text-xs text-gray-600 whitespace-nowrap">
                                          {format(match.dateTime, 'MMM d, h:mma').toLowerCase()}
                                        </div>
                                      )
                                    )}
                                  </div>
                                  <div className="text-sm xs:text-base truncate">
