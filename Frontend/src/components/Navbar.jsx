...
import { Trophy, Bell, User, LogOut, PlusCircle, Calendar, Menu, X, Settings } from 'lucide-react';
import { isLoggedIn, clearAuth } from '../utils/auth';

// No Firebase. Auth state from localStorage JWT.
...
const Navbar = () => {
  ...
  // Re-check auth on every route change (login/logout updates localStorage)
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          {loggedIn ? (
            ...
            <Link to="/login" className="bg-white text-black px-6 py-2.5 rounded-full font-black text-sm hover:bg-[#ccff00] transition-colors active:scale-95">
              SIGN IN
            </Link>
          )}
        </div>
        ...
        {/* Mobile Toggle */}
        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2 text-white bg-white/5 rounded-xl border border-white/10">
                ...
                <div className="flex flex-col gap-4">
                  {loggedIn ? (
                    <>
                      ...
                      <button onClick={handleLogout} className="flex items-center gap-4 p-4 text-red-500 font-bold italic mt-auto">
                        <LogOut size={20} /> LOGOUT
                      </button>
                    </>
                  ) : (
                    <Link to="/login" onClick={() => setIsMenuOpen(false)} className="bg-[#ccff00] text-black p-5 rounded-2xl text-center font-black italic text-xl">
                      SIGN IN NOW
                    </Link>
                  )}
                </div>
